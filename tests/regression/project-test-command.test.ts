...
test('default core graph excludes Playwright and preserves explicit suite order', async () => {
  ...
  assert.deepEqual(testPlan(), ['reader-engine', 'contract', 'regression', 'integration']);
  assert.deepEqual(testPlan('core'), ['reader-engine', 'contract', 'regression', 'integration']);
...
test('test command accepts exactly the approved suites', async () => {
  ...
  for (const suite of ['core', 'reader-engine', 'contract', 'regression', 'integration', 'e2e']) {
