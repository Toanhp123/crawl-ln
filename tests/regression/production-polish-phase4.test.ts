...
test('progress exposes accessible progressbar semantics', () => {
  const source = read('apps/web-legacy/src/shared/ui/feedback/Progress.tsx');
...
test('segmented controls support arrow keyboard navigation', () => {
  const source = read('apps/web-legacy/src/shared/ui/forms/SegmentedControl.tsx');
...
test('reduced motion policy has a single source of truth', () => {
  const appCss = read('apps/web-legacy/src/app/styles/index.css');
  const motionCss = read('apps/web-legacy/src/shared/theme/motion.css');
...
test('reader shell provides a focusable main landmark', () => {
  const shell = read('apps/web-legacy/src/app/layouts/ReaderShell.tsx');
  const viewport = read('apps/web-legacy/src/shared/ui/layout/ScrollViewport.tsx');
