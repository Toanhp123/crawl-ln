...
export interface SwipeChapterNavigationOptions {
  enabled: boolean;
  onPrevious: () => void;
  onNext: () => void;
...
export function useSwipeChapterNavigation({
  enabled,
  onPrevious,
  onNext,
  rootRef,
  rootId = 'reader-content'
}: SwipeChapterNavigationOptions): void {
  ...
  }, [enabled, onNext, onPrevious, rootId, rootRef]);
}
