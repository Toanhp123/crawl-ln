...
test('phase 2 exposes shared typography, list row, card variants, and sticky toolbar', async () => {
  const [index, text, listRow, card, stickyToolbar, toolbar] = await Promise.all([
    read('apps/web-legacy/src/shared/ui/index.ts'),
    read('apps/web-legacy/src/shared/ui/data-display/Text.tsx'),
    read('apps/web-legacy/src/shared/ui/data-display/ListRow.tsx'),
    read('apps/web-legacy/src/shared/ui/layout/Card.tsx'),
    read('apps/web-legacy/src/shared/ui/layout/StickyToolbar.tsx'),
    read('apps/web-legacy/src/shared/ui/layout/Toolbar.tsx')
  ]);
...
test('current settings, source, activity, and library surfaces consume shared primitives', async () => {
  const [settingRow, chapters, taskCard, sourceCard, library] = await Promise.all([
    ...
    read('apps/web-legacy/src/entities/source-plugin/ui/SourcePluginRow.tsx'),
