...
export function webModuleSideEffects(id: string): boolean | undefined {
  const normalizedId = id.replaceAll('\\', '/');
  return normalizedId.startsWith(`${webSourceRoot}/`) &&
    slicedSourceModulePattern.test(normalizedId.slice(webSourceRoot.length))
    ? false
    : undefined;
}
...
export default defineConfig({
  ...
  build: {
    rollupOptions: {
      treeshake: {
        moduleSideEffects: webModuleSideEffects
      }
    }
  },
  server: {
