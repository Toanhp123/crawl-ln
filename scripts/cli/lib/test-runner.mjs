...
export function assertNonEmptySuite(name, files) {
  if (files.length === 0) {
    throw new CommandFailure(`Suite "${name}" discovered zero test files`);
  }
}
...
export async function runTestFile({
  ...
  if (!suite) throw new CommandFailure(`Unknown test suite: ${suiteName}`);
...
export async function runTestSuite(suiteName) {
  ...
  assertNonEmptySuite(suiteName, files);
