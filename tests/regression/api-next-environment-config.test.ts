...
test('api-next default storage is package-relative when launched as a workspace', () => {
  ...
  assert.equal(environment.storageDirectory, resolve(apiRoot, 'storage'));
...
test('api-next resolves relative path overrides against its package root', () => {
  ...
  assert.equal(environment.storageDirectory, resolve(apiRoot, 'runtime-storage'));
  assert.equal(environment.databasePath, resolve(apiRoot, 'runtime-storage/custom.sqlite'));
  assert.equal(environment.sourceReaderPluginDir, resolve(apiRoot, 'runtime-storage/plugins'));
});
