...
test('reader is a nested overlay that keeps novel detail mounted', () => {
  ...
  const detailRoute = read('apps/web-legacy/src/pages/novel-detail/ui/NovelDetailRoute.tsx');
...
test('reader navigation preserves the parent scroll entry and removes legacy return restoration', () => {
  ...
  assert.equal(existsSync('apps/web-legacy/src/shared/navigation/readerReturnState.ts'), true);
});
