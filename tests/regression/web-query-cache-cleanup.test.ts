...
test('task detail and global add flow use canonical task query keys', async () => {
  const keys = await read('apps/web/src/shared/api/queryKeys.ts');
  const overlay = await read('apps/web/src/app/layouts/GlobalAddNovelOverlay.tsx');
  const detail = await read('apps/web/src/pages/task-detail/model/useTaskDetailPage.ts');
...
test('library reuses list data for continue reading and cover failure resets by URL', async () => {
  const library = await read('apps/web/src/pages/library/model/useLibraryPage.ts');
  const cover = await read('apps/web/src/entities/novel/ui/NovelCover.tsx');
...
test('query defaults and maintenance lock use production-safe ownership', async () => {
  const client = await read('apps/web/src/shared/api/queryClient.ts');
  const maintenance = await read('apps/web/src/shared/maintenance/MaintenanceProvider.tsx');
  const keys = await read('apps/web/src/shared/api/queryKeys.ts');
