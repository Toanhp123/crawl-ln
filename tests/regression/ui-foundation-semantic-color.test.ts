...
const colors = read('apps/web/src/shared/theme/colors.css');
const componentTokens = read('apps/web/src/shared/theme/component-tokens.css');
const collectSource = (directory: string): string[] =>
  readdirSync(join(root, directory), { withFileTypes: true }).flatMap((entry) => {
    ...
    if (relative === 'apps/web/src/shared/theme/colors.css') return [];
    return /\.(?:css|ts|tsx)$/.test(entry.name) ? [read(relative)] : [];
  });
const sourceFiles = collectSource('apps/web/src').join('\n');
