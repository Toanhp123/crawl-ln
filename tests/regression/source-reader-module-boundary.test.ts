...
test('app container composes a standalone source reader before crawler', async () => {
  const container = await readFile('apps/api-legacy/src/shared/container/app-container.ts', 'utf8');
  const moduleSource = await readFile(
    'apps/api-legacy/src/shared/container/modules/source-reader.module.ts',
    'utf8'
  );
