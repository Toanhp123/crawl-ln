...
test('app and reader shells own independent scroll viewports', () => {
  const appShell = read('apps/web-legacy/src/app/layouts/AppShell.tsx');
...
test('document scrolling is disabled and smooth behavior is opt-in only', () => {
  const css = read('apps/web-legacy/src/app/styles/index.css');
...
test('reader scroll behavior is scoped to the reader viewport', () => {
  ...
  const progress = read('apps/web-legacy/src/features/read-chapter/model/useReaderProgress.ts');
  const anchors = read('apps/web-legacy/src/features/read-chapter/model/readingAnchor.ts');
...
test('app scroll restoration keeps the background entry stable while reader overlay is open', () => {
  ...
  const navigationState = read('apps/web-legacy/src/shared/navigation/readerReturnState.ts');
