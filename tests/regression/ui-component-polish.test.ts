...
test('settings hub cards align content on the vertical center', () => {
  const source = read('apps/web-legacy/src/pages/settings/ui/SettingsHubCard.tsx');
...
test('crawl and novel detail actions stay in one horizontal row', () => {
  ...
  const management = read('apps/web-legacy/src/pages/novel-detail/ui/NovelManagementSheet.tsx');
...
test('library cards keep reader progress separate from the primary action', () => {
  const source = read('apps/web-legacy/src/entities/novel/ui/NovelLibraryCard.tsx');
...
test('library search and filter controls use canonical square touch targets', () => {
  ...
  const search = read('apps/web-legacy/src/shared/ui/forms/SearchInput.tsx');
