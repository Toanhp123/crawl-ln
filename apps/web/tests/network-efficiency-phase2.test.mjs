...
test('source plugin settings use realtime with a slow disconnected fallback', () => {
  ...
  const pluginBlock = source.slice(source.indexOf('const plugins'), source.indexOf('const runScheduler'));
  assert.match(source, /useRealtimeStatus/);
...
test('task summary endpoint counts polling tasks without loading task rows', () => {
  const routes = readRoot('apps/api/src/modules/task/presentation/routes/task.routes.ts');
  const repository = readRoot(
    'apps/api/src/modules/task/infrastructure/sqlite/task-sqlite.repository.ts'
  );
  ...
  assert.doesNotMatch(repository.slice(repository.indexOf('async countActive'), repository.indexOf('async findRecoverable')), /mapTaskRow/);
});
