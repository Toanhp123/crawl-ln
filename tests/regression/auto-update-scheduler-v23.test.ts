...
test('database migration persists auto update policy and diagnostics', () => {
  const source = read('apps/api-legacy/src/shared/database/sqlite.ts');
...
test('scheduler skips novels with active tasks and applies failure backoff', () => {
  const source = read(
    'apps/api-legacy/src/modules/scheduler/application/auto-update-scheduler.service.ts'
  );
...
test('scheduler API exposes status, manual tick, policy and diagnostics', () => {
  const schedulerRoutes = read(
    'apps/api-legacy/src/modules/scheduler/presentation/scheduler.routes.ts'
  );
  const novelRoutes = read(
    'apps/api-legacy/src/modules/scheduler/presentation/scheduler-novel.routes.ts'
  );
...
test('novel detail and settings expose scheduler controls', () => {
  const novelPage = read('apps/web-legacy/src/pages/novel-detail/ui/NovelDetailPage.tsx');
  const settingsPage = read('apps/web-legacy/src/pages/settings/ui/SettingsPage.tsx');
