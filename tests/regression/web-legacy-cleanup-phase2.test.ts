...
test('runtime navigation no longer targets standalone crawl or tasks pages', () => {
  ...
  const sidebar = read('apps/web-legacy/src/app/layouts/AppSidebar.tsx');
  const tabs = read('apps/web-legacy/src/widgets/bottom-tabs/ui/AppBottomTabs.tsx');
...
test('settings does not load or summarize source plugins', () => {
  const model = read('apps/web-legacy/src/pages/settings/model/useSettingsPage.tsx');
