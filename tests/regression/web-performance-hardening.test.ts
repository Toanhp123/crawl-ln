...
test('library uses server-side pagination instead of slicing the full dataset', () => {
  const model = read('apps/web/src/pages/library/model/useLibraryPage.ts');
  const api = read('apps/web/src/entities/novel/api/novelApi.ts');
...
test('reader disk cache has bounded LRU eviction and quota recovery', () => {
  const source = read('apps/web/src/modules/reader/infrastructure/indexeddb-reader-cache.ts');
