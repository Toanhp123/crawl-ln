...
import { RuntimeContextResolverService } from '../../apps/api-next/src/modules/source-reader/application/services/runtime-context-resolver.service.ts';
...
import { parseSourcePluginManifest } from '../../apps/api-next/src/modules/source-reader/domain/plugin/source-plugin-manifest.schema.ts';
import { browserSessionIdentityKey } from '../../apps/api-next/src/modules/source-reader/infrastructure/browser/browser-runtime.coordinator.ts';
import { buildChromiumLaunchOptions } from '../../apps/api-next/src/modules/source-reader/infrastructure/browser/browser-launch-options.ts';
import { HmacCursorCodec } from '../../apps/api-next/src/modules/source-reader/infrastructure/runtime/hmac-cursor.codec.ts';
import { ExternalProcessSupervisor } from '../../apps/api-next/src/modules/source-reader/infrastructure/runtime/external-process/external-process-supervisor.ts';
import { InProcessPluginRuntime } from '../../apps/api-next/src/modules/source-reader/infrastructure/runtime/in-process/in-process-plugin.runtime.ts';
import { PipelinePluginRegistryAdapter } from '../../apps/api-next/src/modules/source-reader/infrastructure/runtime/pipeline-plugin-registry.adapter.ts';
import { PipelineRuntimeContextAdapter } from '../../apps/api-next/src/modules/source-reader/infrastructure/runtime/pipeline-runtime-context.adapter.ts';
import { PipelineSourceReaderInvocationAdapter } from '../../apps/api-next/src/modules/source-reader/infrastructure/runtime/pipeline-source-reader-invocation.adapter.ts';
import { PluginContextFactory } from '../../apps/api-next/src/modules/source-reader/infrastructure/runtime/plugin-context.factory.ts';
import { RuntimeRouter } from '../../apps/api-next/src/modules/source-reader/infrastructure/runtime/runtime-router.ts';
import { CheerioHtmlParserAdapter } from '../../apps/api-next/src/modules/source-reader/infrastructure/runtime/cheerio-html-parser.adapter.ts';
import { sourceReaderMigrations } from '../../apps/api-next/src/modules/source-reader/infrastructure/migrations/001-source-reader-schema.ts';
import { NetworkRouteResolver } from '../../apps/api-next/src/modules/source-reader/infrastructure/network/network-route.resolver.ts';
import { ProxyAgentFactory } from '../../apps/api-next/src/modules/source-reader/infrastructure/network/proxy-agent.factory.ts';
import { RouteAwareHttpClientAdapter } from '../../apps/api-next/src/modules/source-reader/infrastructure/network/route-aware-http-client.adapter.ts';
import {
  createNovelCoolPlugin,
  novelCoolPlugin
} from '../../apps/api-next/src/modules/source-reader/infrastructure/plugins/built-in/novelcool/novelcool.plugin.ts';
import { SourcePluginPackageVerifier } from '../../apps/api-next/src/modules/source-reader/infrastructure/plugins/package-loader/source-plugin-package.verifier.ts';
import { StaticTrustStore } from '../../apps/api-next/src/modules/source-reader/infrastructure/plugins/package-loader/static-trust.store.ts';
import { InMemoryPluginRegistry } from '../../apps/api-next/src/modules/source-reader/infrastructure/plugins/registry/in-memory-plugin.registry.ts';
import { LocalEncryptedVault } from '../../apps/api-next/src/modules/source-reader/infrastructure/secrets/local-encrypted.vault.ts';
import { SqliteAuthChallengeRepository } from '../../apps/api-next/src/modules/source-reader/infrastructure/sqlite/sqlite-auth-challenge.repository.ts';
import { SqliteCredentialRepository } from '../../apps/api-next/src/modules/source-reader/infrastructure/sqlite/sqlite-credential.repository.ts';
import { SqliteNetworkProfileRepository } from '../../apps/api-next/src/modules/source-reader/infrastructure/sqlite/sqlite-network-profile.repository.ts';
import { PipelinePluginHealthAdapter } from '../../apps/api-next/src/modules/source-reader/infrastructure/sqlite/pipeline-plugin-health.adapter.ts';
import { SqlitePluginHealthRepository } from '../../apps/api-next/src/modules/source-reader/infrastructure/sqlite/sqlite-plugin-health.repository.ts';
import { SqlitePluginStore } from '../../apps/api-next/src/modules/source-reader/infrastructure/sqlite/sqlite-plugin.store.ts';
import { SqliteReaderCache } from '../../apps/api-next/src/modules/source-reader/infrastructure/sqlite/sqlite-reader.cache.ts';
import { SqliteSessionRepository } from '../../apps/api-next/src/modules/source-reader/infrastructure/sqlite/sqlite-session.repository.ts';
import { SqliteDatabase } from '../../apps/api-next/src/platform/database/sqlite-database.ts';
import { copySourceReaderRuntimeAssets } from '../../apps/api-next/scripts/build.mjs';
...
test('api-next build copies the external sandbox entry into its output tree', async (t) => {
  const root = await mkdtemp(resolve(tmpdir(), 'api-next-build-assets-'));
...
test('external sandbox blocks ambient authority and preserves allowlisted SDK errors', async (t) => {
  ...
  const root = await mkdtemp(resolve(tmpdir(), 'api-next-sdk-errors-'));
  t.after(() => rm(root, { recursive: true, force: true }));
...
test('external sandbox rejects oversized RPC values and terminates on cancellation', async (t) => {
  const root = await mkdtemp(resolve(tmpdir(), 'api-next-rpc-bounds-'));
  await mkdir(resolve(root, 'dist'), { recursive: true });
  await writeFile(
    resolve(root, 'dist/index.js'),
    `export async function invokeCapability(payload) {
