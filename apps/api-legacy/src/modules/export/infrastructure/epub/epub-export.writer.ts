...
export class EpubExportWriter implements ExportWriterPort {
  async write(book: ExportBook) {
    ...
    book.chapters.forEach((chapter, i) => {
      ...
      nav.push(`<li><a href="${href}">${xml(chapter.title)}</a></li>`);
      zip.file(
        `OEBPS/${href}`,
    ...
    zip.file(
      'OEBPS/nav.xhtml',
      `<?xml version="1.0" encoding="utf-8"?><html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops"><head><title>Mục lục</title></head><body><nav epub:type="toc"><h1>Mục lục</h1><ol>${nav.join('')}</ol></nav></body></html>`
    );
    zip.file(
      'OEBPS/content.opf',
    ...
    return {
      filename: `${safeName(book.novel.title)}.epub`,
