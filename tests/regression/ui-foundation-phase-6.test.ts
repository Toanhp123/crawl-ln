...
test('shared design system owns reusable switch and icon tile compositions', () => {
  ...
  const switchSource = read('apps/web-legacy/src/shared/ui/forms/Switch.tsx');
  const iconTile = read('apps/web-legacy/src/shared/ui/data-display/IconTile.tsx');
...
test('reader settings reuse the shared switch instead of private toggle variants', () => {
  ...
  const settings = read('apps/web-legacy/src/pages/settings/ui/ReaderSettingsControls.tsx');
...
test('foundation audit removes remaining self-authored typography and bare transitions in audited UI', () => {
  const detail = read('apps/web-legacy/src/pages/novel-detail/ui/NovelDetailPage.tsx');
  const offline = read('apps/web-legacy/src/features/read-chapter/ui/ReaderOfflineBanner.tsx');
...
test('shared UI documentation locks composition and arbitrary-value boundaries', () => {
  const docs = read('apps/web-legacy/src/shared/ui/README.md');
