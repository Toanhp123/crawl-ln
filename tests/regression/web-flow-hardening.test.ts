...
test('bottom navigation derives its columns from the number of items', () => {
  const source = read('apps/web/src/shared/ui/navigation/BottomNav.tsx');
...
test('task polling is disabled by realtime and degrades to a slow disconnected fallback', () => {
  const source = read('apps/web/src/entities/task/model/useTasks.ts');
...
test('activity and library avoid page-owned five-second full-dataset polling', () => {
  const activity = read('apps/web/src/pages/activity/model/useActivityPage.ts');
  ...
  assert.equal(existsSync(new URL('../../apps/web/src/pages/crawl', import.meta.url)), false);
});

test('content search stays inside library and does not run for an empty query', () => {
  const source = read('apps/web/src/features/search-library/model/useSearchLibrary.ts');
...
test('home route redirects synchronously without startup data fetching', () => {
  ...
  const redirect = read('apps/web/src/app/router/HomeRedirect.tsx');
...
test('skip links and automatic synchronization recovery use localized action copy', () => {
  assert.match(read('apps/web/src/app/layouts/AppShell.tsx'), /common\.skipToContent/);
  assert.match(read('apps/web/src/app/layouts/ReaderShell.tsx'), /common\.skipToReader/);
  assert.doesNotMatch(read('apps/web/src/pages/activity/ui/ActivityPage.tsx'), /common\.refresh/);
  assert.match(read('apps/web/src/pages/task-detail/ui/TaskDetailPage.tsx'), /common\.retry/);
});
