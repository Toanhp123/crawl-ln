...
export class SourceReaderInvalidationService implements SourceReaderInvalidationPort {
  constructor(
    ...
    private readonly memoryCache: ReaderCachePort,
    private readonly persistentCache: ReaderCachePort,
  ...
  async invalidate(event: SourceReaderInvalidationEvent): Promise<void> {
    ...
    await this.memoryCache.invalidate(tags);
    await this.persistentCache.invalidate(tags);
