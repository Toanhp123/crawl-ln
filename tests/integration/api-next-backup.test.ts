...
import { createBackupModule } from '../../apps/api-next/src/modules/backup/backup.module.ts';
import { IngestionBackupContributor } from '../../apps/api-next/src/modules/ingestion/infrastructure/backup/ingestion-backup.contributor.ts';
...
import { SchedulerBackupContributor } from '../../apps/api-next/src/modules/scheduler/infrastructure/backup/scheduler-backup.contributor.ts';
import { schedulerMigrations } from '../../apps/api-next/src/modules/scheduler/index.ts';
import { searchMigrations } from '../../apps/api-next/src/modules/search/index.ts';
import { createSearchModule } from '../../apps/api-next/src/modules/search/search.module.ts';
import { SourceReaderBackupContributor } from '../../apps/api-next/src/modules/source-reader/infrastructure/backup/source-reader-backup.contributor.ts';
import { ModuleRegistry } from '../../apps/api-next/src/bootstrap/module-registry.ts';
import { sourceReaderMigrations } from '../../apps/api-next/src/modules/source-reader/index.ts';
...
import type { BackupContributor } from '../../apps/api-next/src/platform/backup/backup-contributor.ts';
...
test('backup merge restores contributor data, settings and search projection', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'novel-tool-v3-backup-merge-'));
  const fixture = await createV22Fixture(root);
  const sourcePath = join(root, 'source-v3.sqlite');
  await importV22Database({ sourcePath: fixture.databasePath, targetPath: sourcePath });
  ...
  const targetPath = join(root, 'target-v3.sqlite');
...
test('backup merge remaps module references onto existing source identities', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'novel-tool-v3-backup-remap-'));
  const fixture = await createV22Fixture(root);
  const sourcePath = join(root, 'source-v3.sqlite');
  await importV22Database({ sourcePath: fixture.databasePath, targetPath: sourcePath });
  const source = new SqliteDatabase(sourcePath);
...
test('invalid contributor data rolls back an otherwise valid merge archive', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'novel-tool-v3-backup-rollback-'));
  const fixture = await createV22Fixture(root);
  const sourcePath = join(root, 'source-v3.sqlite');
  await importV22Database({ sourcePath: fixture.databasePath, targetPath: sourcePath });
  const source = new SqliteDatabase(sourcePath);
...
test('backup replace swaps an offline database and rejects corruption safely', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'novel-tool-v3-backup-replace-'));
  const fixture = await createV22Fixture(root);
  const sourcePath = join(root, 'source-v3.sqlite');
  await importV22Database({ sourcePath: fixture.databasePath, targetPath: sourcePath });
  const source = new SqliteDatabase(sourcePath);
