...
test('non-critical crawler audit writes are published behind an application event boundary', () => {
  ...
  const recoverJobs = read(
    'apps/api-legacy/src/modules/crawler/application/use-cases/recover-crawl-jobs.usecase.ts'
  );
...
test('application event bus is technical infrastructure and event payloads remain module-owned', () => {
  ...
  const crawlerEvent = read(
    'apps/api-legacy/src/modules/crawler/application/events/crawl-audit.event.ts'
  );
  const schedulerEvent = read(
    'apps/api-legacy/src/modules/scheduler/application/events/scheduler-diagnostic.event.ts'
  );
