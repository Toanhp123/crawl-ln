import type { ClockPort, IdGeneratorPort } from '../../../ports/runtime-support.ports.js';
import type {
  CredentialHandle,
...
export class LogoutCredentialUseCase {
  constructor(
    private readonly authorization: SourceReaderAuthorizationPolicy,
    private readonly credentials: Pick<CredentialAdministrationRepository, 'requireHandle'>,
    private readonly authentication: Pick<AuthenticationOrchestratorService, 'logout'>
  ) {}
  async execute(input: { actor: SourceReaderActor; credentialId: string }) {
    this.authorization.requireRole(input.actor, 'source-manager');
    const credential = await this.credentials.requireHandle(input.credentialId);
    this.authorization.assertCredentialAccess(input.actor, credential);
    await this.authentication.logout({ credentialProfileId: input.credentialId });
  }
}
